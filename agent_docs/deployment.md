...
- Vendus credentials are per organization: `vendus_api_key` is write-only to browser roles. Settings validates the entered or saved key through `vendus-options`; the UI only asks for the API key. The legacy `vendus_register_id` and `vendus_payment_method_id` organization columns are cleared when settings are saved and are ignored during issuance. FT, FR, and RG omit `register_id`. For FR, each confirmed `sale_payments.payment_method` is matched to an active Vendus payment method and its amount is allocated to that method; RG uses its source payment's method. Missing, unavailable, or ambiguous methods stop issuance before the fiscal POST. The API key belongs to a Vendus user with document permissions. Apply `20260924120000_keyinvoice_recurring_fiscal_ledger.sql` first, then `20260924130000_vendus_fiscal_provider.sql` in SQL Editor before serving the Vendus frontend. Deploy `vendus-options`, `issue-invoice`, `issue-invoice-receipt`, `generate-receipt`, `get-invoice-details`, `sync-invoices`, `sync-credit-notes`, `cancel-invoice`, `create-credit-note`, and `send-invoice-email` manually to the active project. Validate with Vendus test mode or a designated test account before enabling live issuance for an organization.
