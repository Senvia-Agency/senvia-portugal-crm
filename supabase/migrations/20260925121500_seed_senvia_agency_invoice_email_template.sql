-- Create a branded manual invoice email template for SENVIA Agency.
-- The trigger is consumed by the manual fiscal email flow; automation remains disabled.
DO $$
DECLARE
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE organization_id = v_org
      AND automation_trigger_type = 'invoice_email'
      AND is_active = true
  ) THEN
