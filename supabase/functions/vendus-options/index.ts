...
import { selectNormalVendusRegister, VendusError, vendusRequest } from '../_shared/vendus.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try { selectNormalVendusRegister(registers); registerReady = true }
    catch (error) { readinessError = error instanceof VendusError ? error.message : 'Caixa Vendus inválida.' }
